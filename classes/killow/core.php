...
abstract class Killow_Core 
{
    ...
    {
        $this->zws_id = Kohana::config('killow')->zws_id;
    }
	
	/**
	 * factory - method loads a new Killow object
	 * 
	 */	
	...
	/**
     * Creates (if not already created) and returns a Killow_Transporter object
     * @return Killow_Transporter
     */
    public function create_transporter()
    {
        if( FALSE === isset($this->_transporter) )
            $this->_transporter = new Killow_Transporter;
	 ...
	 * 
     * @param string 	$zpid  - The Zillow Property ID for the property for which to obtain information; the parameter type is an integer
	 ...
	 * 								(AK, AL, AR, AZ, CA, CO, CT, DE, FL, GA, HI, IA, ID, IL, IN, KS, KY, LA, MA, MD, ME, MI, MN, MO, MS, MT, NC, ND, NE, NH, NJ, NM, NV, NY, OH, OK, OR, PA, RI, SC, SD, TN, TX, UT, VA, VT, WA, DC, WI, WV, WY). 
